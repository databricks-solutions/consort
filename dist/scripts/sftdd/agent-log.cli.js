...
// scripts/sftdd/agent-log.cli.ts
init_esm_shims();
...
init_esm_shims();
import { appendFileSync, existsSync as existsSync2, readFileSync as readFileSync3 } from "fs";
...
function loadSchema(name) {
  return JSON.parse(readFileSync2(join2(SCHEMA_DIR, name), "utf8"));
}
...
// scripts/sftdd/log-reconcile.ts
init_esm_shims();
import { existsSync as existsSync6, readdirSync as readdirSync4, statSync as statSync3 } from "fs";
import { join as join5, relative, dirname as dirname3 } from "path";

// scripts/sftdd/architecture-conventions.ts
init_esm_shims();
import { existsSync as existsSync3, readFileSync as readFileSync4, writeFileSync as writeFileSync2, mkdirSync as mkdirSync2 } from "fs";
...
function establishConventionsIfAbsent(sftddDir, featureId, now = () => /* @__PURE__ */ new Date()) {
  ...
  if (!existsSync3(archFile)) return { established: false };
  let content;
  try {
    content = readFileSync4(archFile, "utf8");
  } catch {
...
init_esm_shims();
import { existsSync as existsSync4, readFileSync as readFileSync5, writeFileSync as writeFileSync3, mkdirSync as mkdirSync3, readdirSync as readdirSync2 } from "fs";
...
function deriveCanon(architectureJsonContent, acLayers, featureId, now = () => /* @__PURE__ */ new Date()) {
  let doc;
  try {
    doc = JSON.parse(architectureJsonContent);
  } catch {
  ...
  if (doc.service_backed !== true) return void 0;
  const nfr_posture = [];
  const seenNfr = /* @__PURE__ */ new Set();
  for (const n of doc.nfrs ?? []) {
    if (typeof n.category !== "string" || typeof n.requirement !== "string") continue;
    const key = `${n.category}\0${n.requirement}`;
    if (seenNfr.has(key)) continue;
    seenNfr.add(key);
    nfr_posture.push({ category: n.category, requirement: n.requirement });
  }
  const invariant_patterns = [];
  const seenInv = /* @__PURE__ */ new Set();
  for (const p of doc.persistence_invariants ?? []) {
    if (typeof p.type !== "string") continue;
    if (seenInv.has(p.type)) continue;
    seenInv.add(p.type);
    invariant_patterns.push({ type: p.type, brief: typeof p.brief === "string" ? p.brief : "" });
  }
  return {
    established_by: featureId,
    established_at: now().toISOString(),
    ac_layers: uniq(acLayers),
    nfr_posture,
    invariant_patterns
  };
}
...
function establishCanonFromDisk(sftddDir, featureId, now = () => /* @__PURE__ */ new Date()) {
  ...
  let content;
  try {
    content = readFileSync5(archFile, "utf8");
  } catch {
    return { established: false };
  }
...
// scripts/sftdd/spec-sync.ts
init_esm_shims();
import { readFileSync as readFileSync6, existsSync as existsSync5, readdirSync as readdirSync3, writeFileSync as writeFileSync4, statSync as statSync2 } from "fs";
...
function discoverArtifacts(sftddDir, featureId) {
  ...
  if (!existsSync6(fdir)) return out;
  const add = (abs, role, message) => {
    if (existsSync6(abs)) out.push({ path: relative(sftddDir, abs), role, message });
  };
  ...
  if (existsSync6(sdir)) {
    for (const s of readdirSync4(sdir).sort()) {
      const storyDir2 = join5(sdir, s);
      if (!statSync3(storyDir2).isDirectory()) continue;
      add(join5(storyDir2, "story.json"), "spec-author", `story stub ${s}`);
      const acsDir2 = join5(storyDir2, "acs");
      if (existsSync6(acsDir2)) {
        for (const ac of readdirSync4(acsDir2).sort()) {
          if (ac.endsWith(".json")) {
            add(join5(acsDir2, ac), "spec-author", `AC ${ac.replace(/\.json$/, "")} for story ${s}`);
          }
        }
      }
...
// scripts/sftdd/log-reconstitute.ts
init_esm_shims();
import { existsSync as existsSync7, readFileSync as readFileSync7, writeFileSync as writeFileSync5 } from "fs";
import { join as join6 } from "path";
function sig(e) {
  const m = e.metadata ?? {};
  return JSON.stringify([e.role, e.event, e.message, m.feature_id ?? "", m.story ?? "", m.path ?? m.artifact ?? ""]);
}
function ts(e) {
...
function writeSmellsLog(sftddDir, hits) {
  ...
  const existing = existsSync8(file) ? JSON.parse(readFileSync8(file, "utf8")) : { detected: [] };
...
import { existsSync as existsSync10, mkdirSync as mkdirSync5, readFileSync as readFileSync11, rmSync as rmSync2, writeFileSync as writeFileSync8 } from "fs";
  ...
  for (let i = 0; i < argv.length; i++) {
    switch (argv[i]) {
      ...
      case "--reconcile":
        ...
        break;
      case "--reconstitute":
        out.reconstitute = true;
        break;
      case "--design-log":
        out.designLog = argv[++i];
        break;
      case "--role":
        out.role = argv[++i];
        break;
...
`);
  ...
  if (a.events !== void 0) {
    ...
    try {
      ...
      process.stderr.write(`Error: --events is not valid JSON: ${e.message}
`);
      return 2;
    }
...
`);
      ...
      if (typeof el.data === "string") {
        try {
          Object.assign(slots2, JSON.parse(el.data));
        } catch (e) {
          process.stderr.write(`Error: an --events item's data is not valid JSON: ${e.message}
`);
          return 2;
        }
      }
      inputs.push({
        ...
        level: el.level,
        event: el.event,
        feature_id: typeof el.feature === "string" ? el.feature : void 0,
        phase: typeof el.phase === "string" ? el.phase : void 0,
        cycle_id: typeof el.cycle === "string" ? el.cycle : void 0,
        slots: slots2
      });
    }
    try {
