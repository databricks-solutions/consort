...
init_esm_shims();
import { existsSync as existsSync3, readFileSync as readFileSync4, writeFileSync as writeFileSync2, mkdirSync as mkdirSync2, readdirSync as readdirSync3, statSync as statSync3, rmSync } from "fs";
...
function storyAcProblems(fdir, story) {
  ...
  const problems = [];
  const acs = [];
  for (const f of readdirSync2(acsDir2)) {
    if (!f.endsWith(".json")) continue;
    const p = join4(acsDir2, f);
    let content;
    try {
...
function readPipeline(sftddDir, featureId) {
  ...
  if (!existsSync3(p)) return initPipeline(featureId);
...
function syncBreakdownToPipeline(sftddDir, featureId) {
  ...
  const added = [];
  if (existsSync3(storiesDir2)) {
    for (const storyId of readdirSync3(storiesDir2).sort()) {
      let isDir = false;
      try {
        isDir = statSync3(join5(storiesDir2, storyId)).isDirectory();
      } catch {
        isDir = false;
      }
      if (!isDir) continue;
...
function storyHasAcceptanceCriteria(sftddDir, featureId, storyId) {
  ...
  if (!existsSync3(acsDir2)) return false;
...
function findBatchedDraftStories(sftddDir, featureId, pipeline, gatingStoryId) {
  const storiesDir2 = storiesDir(sftddDir, featureId);
  if (!existsSync3(storiesDir2)) return [];
  const offenders = [];
...
init_esm_shims();
import { existsSync as existsSync14, readFileSync as readFileSync16, writeFileSync as writeFileSync10, mkdirSync as mkdirSync9, readdirSync as readdirSync10, rmSync as rmSync7 } from "fs";
...
function resolveAllOpenSmellsForStory(sftddDir, story, note) {
  ...
  if (cleared.length) writeFileSync3(file, JSON.stringify(log, null, 2) + "\n");
  return cleared;
}

// scripts/sftdd/reflection.ts
init_esm_shims();
import { existsSync as existsSync6, readFileSync as readFileSync7, writeFileSync as writeFileSync4, mkdirSync as mkdirSync3, rmSync as rmSync2 } from "fs";
var SMELL_FOR_OWNER = {
...
function clearReflectVerdict(sftddDir, feature, story) {
  ...
  if (existsSync6(p)) rmSync2(p, { force: true });
}
var REFLECT_SMELLS = Object.values(SMELL_FOR_OWNER);

// scripts/sftdd/cycle-record.ts
init_esm_shims();
import { existsSync as existsSync13, readFileSync as readFileSync15, readdirSync as readdirSync9, statSync as statSync7, writeFileSync as writeFileSync9, mkdirSync as mkdirSync8, rmSync as rmSync6 } from "fs";
...
import { existsSync as existsSync9, mkdirSync as mkdirSync6, readFileSync as readFileSync11, rmSync as rmSync4, writeFileSync as writeFileSync7 } from "fs";
...
init_esm_shims();
import { existsSync as existsSync11, readFileSync as readFileSync13, readdirSync as readdirSync7, statSync as statSync5 } from "fs";
import { join as join12, relative, extname } from "path";

// scripts/sftdd/migration-app-clean.ts
init_esm_shims();
import { existsSync as existsSync12, readFileSync as readFileSync14, readdirSync as readdirSync8, statSync as statSync6 } from "fs";
...
function staleStoryArtifactsForRevise(sftddDir, featureId, story, gate) {
  ...
  if (existsSync14(perStory)) rmSync7(perStory, { force: true });
  if (gate === "spec") {
    const dir = acsDir(sftddDir, featureId, story);
    if (existsSync14(dir)) {
      for (const f of readdirSync10(dir)) {
        if (f.endsWith(".json") || f.endsWith(".md")) rmSync7(join15(dir, f), { force: true });
      }
    }
      ...
      for (const f of readdirSync10(dir)) {
        ...
        try {
          ...
          if ("architectural_notes" in ac) {
            ...
            writeFileSync10(p, JSON.stringify(ac, null, 2) + "\n");
          }
        } catch {
        }
      }
...
function applyReviseSelfHeal(args) {
  ...
  try {
    ...
    writeFileSync10(hb, composeReviseBrief({ smell: args.smell, gate: args.gate, reason: args.reason }));
  } catch {
  }
...
function resolveKitBinJs(bin) {
  if (kitBinMap === null) {
    try {
      const pkg = JSON.parse(fs5.readFileSync(path3.join(KIT_ROOT, "package.json"), "utf8"));
      kitBinMap = pkg.bin ?? {};
    } catch {
      kitBinMap = {};
    }
