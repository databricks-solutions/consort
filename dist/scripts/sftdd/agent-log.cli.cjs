...
var agent_log_cli_exports = {};
__export(agent_log_cli_exports, {
  runAgentLogCli: () => runAgentLogCli
});
...
function emitAgentLogEvent(input, opts = {}) {
  ...
  (0, import_fs2.appendFileSync)(logFilePath(sftddDir), `${JSON.stringify(event)}
`, "utf8");
  return event;
}
function emitAgentLogEvents(inputs, opts = {}) {
  ...
  (0, import_fs2.appendFileSync)(logFilePath(sftddDir), events.map((e) => `${JSON.stringify(e)}
`).join(""), "utf8");
  return events;
}
function readAgentLog(opts = {}) {
  ...
  const out = [];
  for (const line of (0, import_fs2.readFileSync)(file, "utf8").split("\n")) {
...
function readCanon(sftddDir) {
  ...
  if (!(0, import_fs4.existsSync)(f)) return void 0;
  try {
...
function establishCanonFromDisk(sftddDir, featureId, now = () => /* @__PURE__ */ new Date()) {
  ...
  const archFile = architectureJson(sftddDir, featureId);
  if (!(0, import_fs4.existsSync)(archFile)) return { established: false };
  let content;
  try {
    content = (0, import_fs4.readFileSync)(archFile, "utf8");
  } catch {
    return { established: false };
  }
...
function writeCanon(sftddDir, canon) {
  ...
  (0, import_fs4.mkdirSync)((0, import_path4.dirname)(out), { recursive: true });
  (0, import_fs4.writeFileSync)(out, JSON.stringify(canon, null, 2) + "\n");
}
...
function discoverArtifacts(sftddDir, featureId) {
  ...
  if (!(0, import_fs6.existsSync)(fdir)) return out;
  const add = (abs, role, message) => {
    if ((0, import_fs6.existsSync)(abs)) out.push({ path: (0, import_path6.relative)(sftddDir, abs), role, message });
  };
  add((0, import_path6.join)(fdir, "feature-spec.json"), "spec-author", "feature-spec.json");
  ...
  const designDir = (0, import_path6.dirname)(designGuideJson(sftddDir));
  ...
  if ((0, import_fs6.existsSync)(sdir)) {
    for (const s of (0, import_fs6.readdirSync)(sdir).sort()) {
      const storyDir2 = (0, import_path6.join)(sdir, s);
      if (!(0, import_fs6.statSync)(storyDir2).isDirectory()) continue;
      add((0, import_path6.join)(storyDir2, "story.json"), "spec-author", `story stub ${s}`);
      const acsDir2 = (0, import_path6.join)(storyDir2, "acs");
      if ((0, import_fs6.existsSync)(acsDir2)) {
        for (const ac of (0, import_fs6.readdirSync)(acsDir2).sort()) {
          if (ac.endsWith(".json")) {
            add((0, import_path6.join)(acsDir2, ac), "spec-author", `AC ${ac.replace(/\.json$/, "")} for story ${s}`);
          }
        }
      }
...
function readJsonl(file) {
  ...
  const out = [];
  for (const line of (0, import_fs7.readFileSync)(file, "utf8").split("\n")) {
    if (line.trim().length === 0) continue;
    try {
...
function reconstituteAgentLog(opts) {
  const projectLog = (0, import_path7.join)(opts.sftddDir, "agent-log.jsonl");
  ...
  (0, import_fs7.writeFileSync)(projectLog, final.map((e) => JSON.stringify(e)).join("\n") + "\n", "utf8");
  return final;
}
...
function writeSmellsLog(sftddDir, hits) {
  const file = (0, import_path8.join)(sftddDir, "smells.json");
...
function readSmellsLog(sftddDir) {
  const file = (0, import_path8.join)(sftddDir, "smells.json");
  if (!(0, import_fs8.existsSync)(file)) return { detected: [] };
