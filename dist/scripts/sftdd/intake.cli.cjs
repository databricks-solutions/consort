...
// scripts/sftdd/intake.cli.ts
var intake_cli_exports = {};
__export(intake_cli_exports, {
...
function readAgentConfig(projectDir) {
  ...
  if (!(0, import_fs2.existsSync)(p)) return void 0;
...
function loadSftddConfig(projectDir) {
  for (const rel of [SFTDD_CONFIG_REL, LEGACY_TDD_CONFIG_REL]) {
    ...
    if (!(0, import_fs3.existsSync)(f)) continue;
    try {
...
function checkIntakePreconditions(args = {}) {
  ...
  const projectDir = args.projectDir ?? (0, import_node_path2.dirname)(sftddDir);
  const uiTrack = resolveSftddSettings({ projectDir }).project.uiTrack;
  const required = [
    { artifact: "product-overview.md", path: (0, import_node_path2.join)(sftddDir, "product-overview.md") },
    { artifact: "nfrs.md", path: (0, import_node_path2.join)(sftddDir, "nfrs.md") }
  ];
  if (uiTrack) {
    required.push({ artifact: "design-brief.md", path: (0, import_node_path2.join)(sftddDir, "design", "design-brief.md") });
  }
  if (args.featureId) {
