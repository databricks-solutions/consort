...
function metaNum(ev, key) {
  const v = ev.metadata?.[key];
  return typeof v === "number" && !Number.isNaN(v) ? v : void 0;
}
function rollupTurns(turns, keyOf) {
...
function computeTiming(events, opts = {}) {
  ...
  let turnUsage = events.filter((ev) => ev.event === "turn.usage").map((ev) => {
...
function turnRollupBlock(title, rows) {
  if (rows.length === 0) return `${title}
  (none)
`;
  const keyW = Math.max(title.length, ...rows.map((r) => r.key.length));
  const lines = rows.map(
    (r) => `  ${r.key.padEnd(keyW)}  ${String(r.count).padStart(3)}x  avg ${fmtSecs(r.avgSeconds).padStart(8)}  max ${fmtSecs(r.maxSeconds).padStart(8)}  $${(r.costUsd ?? 0).toFixed(2)}`
  );
  return `${title}
${lines.join("\n")}
`;
...
function formatTimingReport(report) {
  ...
  if (report.turnUsage.length > 0) {
    ...
    out.push(
      `MEASURED turns (turn.usage): ${report.turnUsage.length} turns, ${fmtSecs(report.turnSeconds)} compute, $${report.turnCostUsd.toFixed(2)}${planNote}`
    );
...
var HELP = `lakebase-sftdd-timing
