...
init_esm_shims();
import { existsSync as existsSync3, readFileSync as readFileSync4, mkdirSync as mkdirSync3, writeFileSync as writeFileSync3 } from "fs";
...
function readAgentConfig(projectDir) {
  const p = join3(projectDir, AGENT_CONFIG_REL);
...
function checkIntakePreconditions(args = {}) {
  ...
  const statuses = required.map(({ artifact, path: path2 }) => {
    ...
    const result = checkArtifactConformance(artifact, readFileSync5(path2, "utf8"));
    return {
      artifact,
      path: path2,
      present: true,
