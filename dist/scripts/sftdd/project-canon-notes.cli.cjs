...
// scripts/sftdd/project-canon-notes.cli.ts
init_cjs_shims();
...
function readCanon(sftddDir2) {
  ...
  if (!(0, import_fs.existsSync)(f)) return void 0;
  try {
...
function evaluateStoryCanon(sftddDir2, featureId, story) {
  ...
  let archContent;
  if ((0, import_fs.existsSync)(archFile)) {
    try {
      archContent = (0, import_fs.readFileSync)(archFile, "utf8");
    } catch {
      archContent = void 0;
    }
  }
  return assertCanonCoverage(canon, layers, archContent);
}
function architectNovelty(canon, storyAcs, storyArchitectureJsonContent) {
  ...
  if (storyArchitectureJsonContent) {
    ...
    if (doc) {
      const knownInv = new Set(canon.invariant_patterns.map((p2) => p2.type));
      for (const t of uniq((doc.persistence_invariants ?? []).map((p2) => p2.type ?? ""))) {
        if (!knownInv.has(t)) reasons.push(`persistence-invariant type "${t}" is not a canon pattern`);
      }
      const knownCat = new Set(canon.nfr_posture.map((n2) => n2.category));
      for (const c of uniq((doc.nfrs ?? []).map((n2) => n2.category ?? ""))) {
...
function assertCanonCoverage(canon, storyAcs, storyArchitectureJsonContent) {
  const nv = architectNovelty(canon, storyAcs, storyArchitectureJsonContent);
  return nv.novel ? { ok: false, gaps: nv.reasons } : { ok: true };
}
function projectArchitecturalNotes(canon, ac) {
  if (typeof ac.layer !== "string" || ac.layer.length === 0) return void 0;
  const posture = canon.nfr_posture.map((n2) => n2.category);
...
function projectStoryNotes(sftddDir2, featureId, story) {
  ...
  for (const acId of storyAcIds(sftddDir2, featureId, story)) {
    ...
    let ac;
    try {
      ac = JSON.parse((0, import_fs.readFileSync)(file, "utf8"));
    } catch {
      continue;
    }
    if (typeof ac.architectural_notes === "string" && ac.architectural_notes.trim().length > 0) continue;
    const note = projectArchitecturalNotes(canon, { layer: typeof ac.layer === "string" ? ac.layer : void 0 });
    if (!note) continue;
    ac.architectural_notes = note;
    (0, import_fs.writeFileSync)(file, JSON.stringify(ac, null, 2) + "\n");
    annotated++;
  }
  return annotated;
}
...
function writeSmellsLog(sftddDir2, hits) {
  const file = (0, import_path2.join)(sftddDir2, "smells.json");
  const existing = (0, import_fs2.existsSync)(file) ? JSON.parse((0, import_fs2.readFileSync)(file, "utf8")) : { detected: [] };
  const ts = (/* @__PURE__ */ new Date()).toISOString();
  const newEntries = hits.map((h) => ({ ...h, detected_at: ts }));
  const merged = { detected: [...existing.detected, ...newEntries] };
  (0, import_fs2.writeFileSync)(file, JSON.stringify(merged, null, 2) + "\n");
  return merged;
}

// scripts/sftdd/project-canon-notes.cli.ts
function parse(argv) {
  const out = { feature: "", story: "" };
  ...
  writeSmellsLog(sftddDir, [
    {
      ...
      cycle_ids: [],
      story_id: p.story,
      detail: `Canon does not cover ${p.feature}/${p.story}: ${coverage.gaps.join("; ")}. Route to the Architect to annotate the uncovered ACs + amend the canon.`
    }
  ]);
  process.stdout.write(
