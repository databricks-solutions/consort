...
var approve_gate_cli_exports = {};
__export(approve_gate_cli_exports, {
  runApproveGateCli: () => runApproveGateCli
});
module.exports = __toCommonJS(approve_gate_cli_exports);
init_cjs_shims();
...
function readSprintGates(sprint, opts = {}) {
  ...
  if (!(0, import_node_fs.existsSync)(file)) return defaultSprintGatesState(sprint);
  let parsed;
  try {
...
function writeSprintGates(state, opts = {}) {
  const consortDir = opts.consortDir ?? resolveConsortDir();
  (0, import_node_fs.mkdirSync)(sprintDir(consortDir, state.sprint), { recursive: true });
  const file = sprintGatesJson(consortDir, state.sprint);
  const tmp = `${file}.tmp.${process.pid}.${Date.now()}`;
  (0, import_node_fs.writeFileSync)(tmp, JSON.stringify(state, null, 2) + "\n", "utf8");
  try {
...
function writeGates(state, opts = {}) {
  ...
  (0, import_fs5.writeFileSync)(tempFile, payload, "utf8");
  try {
    (0, import_fs5.renameSync)(tempFile, file);
  } catch (err) {
    try {
      (0, import_fs5.unlinkSync)(tempFile);
    } catch {
    }
    throw err;
  }
...
function appendSelectionLog(consortDir, entry) {
  const logPath = (0, import_path6.join)(consortDir, "selection-log.md");
  ...
  if ((0, import_fs6.existsSync)(logPath)) {
    (0, import_fs6.writeFileSync)(logPath, (0, import_fs6.readFileSync)(logPath, "utf8") + text);
  } else {
    (0, import_fs6.writeFileSync)(logPath, text);
  }
}
...
function clientKindLayerReason(consortDir, featureId, testListJson) {
  ...
  if (!(0, import_node_fs3.existsSync)(storiesDir2)) return null;
  const acLayerById = {};
  for (const story of (0, import_node_fs3.readdirSync)(storiesDir2)) {
    const acsDir2 = (0, import_node_path3.join)(storiesDir2, story, "acs");
    if (!(0, import_node_fs3.existsSync)(acsDir2)) continue;
    for (const f of (0, import_node_fs3.readdirSync)(acsDir2)) {
      if (!f.endsWith(".json")) continue;
      try {
        const layer = JSON.parse((0, import_node_fs3.readFileSync)((0, import_node_path3.join)(acsDir2, f), "utf8")).layer;
...
function acReferenceReason(consortDir, featureId, testListJson) {
  const storiesDir2 = (0, import_node_path3.join)(featureResolved(consortDir, featureId), "stories");
  if (!(0, import_node_fs3.existsSync)(storiesDir2)) return null;
