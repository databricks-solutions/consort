...
function discoverArtifacts(consortDir, featureId) {
  ...
  const add = (abs, role, message) => {
    if ((0, import_fs6.existsSync)(abs)) out.push({ path: (0, import_path6.relative)(consortDir, abs), role, message });
  };
...
// consort/logging/log-reconstitute.ts
init_cjs_shims();
...
function reconstituteAgentLog(opts) {
  const projectLog = (0, import_path7.join)(opts.consortDir, "agent-log.jsonl");
