...
function storyCycles(consortDir, featureId, story) {
  const base = join16(cyclesRootDir(consortDir), featureId, story);
...
async function greenOpenCycle(args) {
  ...
  let result = await verify({ projectDir: dirname6(consortDir), consortDir, featureId, story, branchId: open.branch_id, cycleLayer: open.layer });
...
function flagUxAdherenceIfDirty(consortDir, story) {
  try {
    ...
    try {
      ...
      if (existsSync16(gp)) {
        const guide = JSON.parse(readFileSync17(gp, "utf8"));
