...
var human_proxy_cli_exports = {};
__export(human_proxy_cli_exports, {
  runHumanProxyCli: () => runHumanProxyCli
});
module.exports = __toCommonJS(human_proxy_cli_exports);
init_cjs_shims();
...
function gatesFilePath(consortDir, featureId) {
  return (0, import_path2.join)(requireFeatureDir(consortDir, featureId), "gates.json");
}
...
function appendSelectionLog(consortDir, entry) {
  const logPath = (0, import_path3.join)(consortDir, "selection-log.md");
...
function resolveSchemaDir() {
  ...
  if ((0, import_fs4.existsSync)(direct)) return direct;
  let dir = __dirname;
  for (let i = 0; i < 8; i++) {
    const cand = (0, import_path4.join)(dir, "consort", "config", "schemas");
    if ((0, import_fs4.existsSync)(cand)) return cand;
...
function supplyArtifact(args) {
  const approver = args.approver ?? HUMAN_PROXY;
  const artifact = args.artifact ?? (0, import_node_path3.basename)(args.to);
  const consortDir = args.consortDir ?? resolveConsortDir();
  const refuse = (reason) => {
    try {
      emitAgentLogEvent(
  ...
  if (!(0, import_node_fs2.existsSync)(args.from)) {
    return refuse(`recorded source not found: ${args.from}`);
  }
  const content = (0, import_node_fs2.readFileSync)(args.from, "utf8");
  ...
  (0, import_node_fs2.mkdirSync)((0, import_node_path3.dirname)(args.to), { recursive: true });
...
function summarizeRequest(from) {
  let text = "";
  try {
    text = (0, import_node_fs2.readFileSync)(from, "utf8");
  } catch {
...
` + sections.join("\n");
  (0, import_node_fs2.mkdirSync)(planningDir(consortDir), { recursive: true });
  (0, import_node_fs2.writeFileSync)(featureProposalsMd(consortDir), body);
...
function readSmellsLog(consortDir) {
  ...
  if (!(0, import_fs12.existsSync)(file)) return { detected: [] };
  return JSON.parse((0, import_fs12.readFileSync)(file, "utf8"));
}
...
function markSmellResolved(consortDir, smell, opts) {
  const file = (0, import_path12.join)(consortDir, "smells.json");
  if (!(0, import_fs12.existsSync)(file)) return false;
...
function storyTestListFingerprint(consortDir, featureId, story_id) {
  ...
  try {
    return (0, import_crypto2.createHash)("sha1").update((0, import_fs12.readFileSync)(f)).digest("hex");
  } catch {
    return "";
  }
}
function resolveOpenReflectSmellsForStory(consortDir, story_id, note, artifactSha) {
  const file = (0, import_path12.join)(consortDir, "smells.json");
  if (!(0, import_fs12.existsSync)(file)) return 0;
  const log = JSON.parse((0, import_fs12.readFileSync)(file, "utf8"));
  let n = 0;
  ...
  if (n) (0, import_fs12.writeFileSync)(file, JSON.stringify(log, null, 2) + "\n");
  return n;
}
...
function clearReflectVerdict(consortDir, feature, story) {
  ...
  if ((0, import_fs13.existsSync)(p)) (0, import_fs13.rmSync)(p, { force: true });
}
...
function resetStoryBuildState(consortDir, featureId, story) {
  const cyclesDir = (0, import_path13.join)(cyclesRootDir(consortDir), featureId, story);
  let cyclesCleared = false;
  if ((0, import_fs14.existsSync)(cyclesDir)) {
    (0, import_fs14.rmSync)(cyclesDir, { recursive: true, force: true });
    cyclesCleared = true;
  }
  let testItemsReset = 0;
  const tlPath = storyTestListJson(consortDir, featureId, story);
  if ((0, import_fs14.existsSync)(tlPath)) {
    try {
      const tl = JSON.parse((0, import_fs14.readFileSync)(tlPath, "utf8"));
      ...
      (0, import_fs14.writeFileSync)(tlPath, JSON.stringify(tl, null, 2) + "\n");
    } catch {
    }
  }
