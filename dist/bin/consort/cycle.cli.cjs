...
var EPHEMERAL_VERIFY_TTL = "3600s";
async function withEphemeralVerifyBranch(args, run) {
  const ttl = args.ttl ?? EPHEMERAL_VERIFY_TTL;
  const create = args.create ?? (async (a) => {
    await (0, import_lakebase3.createBranch)({ instance: a.instance, branch: a.branch, parentBranch: a.parentBranch, ttl: a.ttl });
  });
  const waitReady = args.waitReady ?? (async (a) => {
    await (0, import_lakebase5.waitForBranchAuthReady)({ instance: a.instance, branch: a.branch });
  });
  const resolveDsn = args.resolveDsn ?? (async (a) => (await (0, import_lakebase5.getConnection)({ instance: a.instance, branch: a.branch, database: a.database, output: "dsn" })).url);
  const remove = args.remove ?? (async (a) => {
    await (0, import_lakebase4.deleteBranch)({ instance: a.instance, branch: a.branch });
  });
...
function ephemeralVerifyBranchName(experimentBranch, nonce) {
  const clean = (s) => s.replace(/[^a-zA-Z0-9-]/g, "-").replace(/-+/g, "-").replace(/^-|-$/g, "");
  const suffix = `-vrfy-${clean(nonce)}`;
  const prefixBudget = Math.max(0, import_util.LAKEBASE_BRANCH_NAME_MAX - suffix.length);
...
function readAppDatabaseName(projectDir) {
  ...
  const urlLine = env.split("\n").map((l) => l.trim()).filter((l) => /^DATABASE_URL\s*=\s*\S/.test(l)).pop();
  if (urlLine) {
...
function resolveDeployTarget(projectDir, name) {
  const cfg = (0, import_lakebase6.readTargets)(projectDir);
...
async function ensureDeployedAndVerify(args) {
  ...
  try {
    if (isPython) {
      ...
      const migRun = mainPassed ? await runVerifyMaybeEphemeral(
        runVerify,
        cfg.verify,
        args.projectDir,
...
function computeTreeState(projectDir) {
  try {
    const headSha = (0, import_node_child_process3.execFileSync)("git", ["rev-parse", "HEAD"], { cwd: projectDir, encoding: "utf8" }).trim();
    const porcelain = (0, import_node_child_process3.execFileSync)("git", ["status", "--porcelain"], { cwd: projectDir, encoding: "utf8" });
    ...
    const dirtySha = (0, import_node_crypto2.createHash)("sha1").update(codeLines.join("\n")).digest("hex");
...
function writeGreenFailure(tdd, feature, story, ac, value) {
  ...
  fs4.writeFileSync(greenFailureJson(tdd, feature, story, ac), JSON.stringify(stamped, null, 2) + "\n");
}
function clearGreenFailure(tdd, feature, story, ac) {
  fs4.rmSync(greenFailureJson(tdd, feature, story, ac), { force: true });
}
...
function regressionFixExhausted(gf) {
  return (gf.fixAttempts ?? 0) >= MAX_REGRESSION_FIX_ATTEMPTS;
}
function composeAssessedGreenFailure(prior, regression) {
  return {
    assessed: true,
...
function rearmRegressionFix(tdd, feature, story, ac, fresh) {
  ...
  fs4.rmSync(regressionAssessmentJson(tdd, feature, story, ac), { force: true });
...
function writeRegressionAssessment(tdd, feature, story, ac, value) {
  fs4.mkdirSync(cycleDir(tdd, feature, story, ac), { recursive: true });
  fs4.writeFileSync(regressionAssessmentJson(tdd, feature, story, ac), JSON.stringify(value, null, 2) + "\n");
}
...
function supersededTestCandidates(args) {
  ...
  const advisory = `SUPERSEDED-TEST CANDIDATES (pre-localized; you do NOT need to search): the migration DROPPED ${syms}, and these PRIOR test lines still assert it, so the new AC supersedes them. Flag EXACTLY these test file(s) as superseded (path (a)) so the Driver may permissively refactor them in the SAME repair turn as the code fix. Do NOT hand-edit them to force green:
${list}`;
...
function walk2(dir, keep, out = []) {
  ...
  for (const e of entries) {
    ...
    if (st.isDirectory()) {
      if (!EXCLUDE_DIR2.test(abs)) walk2(abs, keep, out);
    } else if (st.isFile() && keep(abs)) {
      out.push(abs);
    }
  }
...
function git(args, cwd) {
  return (0, import_node_child_process4.execFileSync)("git", args, { cwd, encoding: "utf8" }).trim();
}
function gitOk(args, cwd) {
  try {
    (0, import_node_child_process4.execFileSync)("git", args, { cwd, stdio: "ignore" });
    return true;
  } catch {
    return false;
  }
...
function walk3(dir, keep, out = []) {
  ...
  for (const e of entries) {
    ...
    if (st.isDirectory()) {
      if (!EXCLUDE_DIR3.test(abs)) walk3(abs, keep, out);
    } else if (st.isFile() && keep(abs)) {
      out.push(abs);
    }
  }
...
async function commitExperimentCode(projectDir, message) {
  ...
  return (0, import_git.commitAllIfChanged)({
    cwd: projectDir,
    message,
...
function isProvisioningFaultSummary(summary) {
  if (!summary) return false;
  ...
  if (!result.passed && !consortEnv("REPLAY_BUILD_DIR") && isProvisioningFaultSummary(result.summary)) {
    ...
    if (retry.passed) {
      ...
      const escalation = writeEscalation(consortDir, {
        ...
        reason: `The verify of ${open.test_id} (${open.ac_id}) in ${featureId}/${story} fails with a DB-provisioning fault (the E2E app is serving an unmigrated or unreachable database: UndefinedTable / relation does not exist / connection refused), and a fresh-boot re-verify failed the same way. This is an INFRASTRUCTURE fault, NOT an app-code regression \u2013 do NOT repair app code. Remediate the provisioning: migrate the branch the app serves (\`uv run alembic upgrade head\` against the served branch's DSN), kill any stale server on the E2E ports (\`lsof -ti tcp:8000,5173 | xargs kill\`), then re-run the verify. (verify: ${retry.summary})`,
        feature_id: featureId,
        story_id: story,
        ac_id: open.ac_id
      });
...
function recordReflectionGate(consortDir, feature, story) {
  ...
  const hits = [...owners].map((owner) => {
    const detail = blocking.filter((f) => f.owner === owner).map((f) => f.detail).join("; ");
    return {
