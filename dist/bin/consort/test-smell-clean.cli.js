...
function collectKnownTables(projectDir, migrationDirs) {
  ...
  for (const md of migrationDirs) {
    ...
    for (const file of walk(abs, (p2) => [".py", ".sql", ".js", ".ts"].includes(extname(p2)))) {
      let body;
      try {
...
function collectDroppedColumns(projectDir, migrationDirs) {
  ...
  for (const md of migrationDirs) {
    const abs = join2(projectDir, md);
    if (!existsSync2(abs)) continue;
    for (const f of walk(abs, (p2) => [".py", ".sql"].includes(extname(p2)))) files.push(f);
  }
  files.sort();
...
function checkDroppedColumnDanglingRef(projectDir, migrationDirs) {
  ...
  const out = [];
  const migAbs = migrationDirs.map((d) => join2(projectDir, d));
...
function checkReversibleInvariantRoundTrip(projectDir) {
  ...
  const out = [];
  if (!existsSync2(featsDir)) return out;
  for (const feature of readdirSync2(featsDir)) {
    const archPath = architectureJson(tdd, feature);
    if (!existsSync2(archPath)) continue;
    let arch;
    try {
      arch = JSON.parse(readFileSync2(archPath, "utf8"));
    } catch {
      continue;
    }
    ...
    const itemSources = [join2(featureDir(tdd, feature), "test-list.json")];
    const stDir = storiesDir(tdd, feature);
    if (existsSync2(stDir)) {
      for (const s of readdirSync2(stDir)) {
        const p2 = storyTestListJson(tdd, feature, s);
        if (existsSync2(p2)) itemSources.push(p2);
      }
    }
    const items = [];
    for (const src of itemSources) {
      try {
        const tl = JSON.parse(readFileSync2(src, "utf8"));
        items.push(...tl.items ?? []);
      } catch {
      }
    }
...
function checkTestSmells(args) {
  ...
  for (const td of testDirs) {
    const abs = join2(args.projectDir, td);
    if (!existsSync2(abs)) continue;
