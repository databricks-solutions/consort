...
function readSmellsLog(consortDir) {
  const file = join3(consortDir, "smells.json");
  if (!existsSync3(file)) return { detected: [] };
...
function resolveOpenSmells(consortDir, smell, opts) {
  ...
  const log = JSON.parse(readFileSync3(file, "utf8"));
  let n = 0;
  ...
  if (n) writeFileSync2(file, JSON.stringify(log, null, 2) + "\n");
  return n;
}
...
import { existsSync as existsSync5, mkdirSync as mkdirSync3, readFileSync as readFileSync6, rmSync as rmSync2, writeFileSync as writeFileSync4 } from "fs";
...
init_esm_shims();
import { existsSync as existsSync12, readFileSync as readFileSync12, readdirSync as readdirSync7, statSync as statSync5 } from "fs";
...
function storyCycles(consortDir, featureId, story) {
  const base = join14(cyclesRootDir(consortDir), featureId, story);
  if (!existsSync13(base)) return [];
  const out = [];
  for (const acDir of readdirSync8(base)) {
    ...
    for (const f of readdirSync8(dir)) {
      ...
      try {
        out.push(JSON.parse(readFileSync13(join14(dir, f), "utf8")));
      } catch {
      }
    }
  }
...
function resolveEscalations(consortDir, opts = {}) {
  ...
  for (const e of readEscalations(consortDir)) {
    ...
    if (opts.featureId && e.feature_id !== void 0 && e.feature_id !== opts.featureId) continue;
...
function parseArgs(argv) {
  const out = { projectDir: process.cwd(), all: false, list: false };
