...
async function deleteSpike(args) {
  ...
  if (purgeNotes) (0, import_fs.rmSync)(dir, { recursive: true, force: true });
}
...
var newTraceId = () => (0, import_node_crypto.randomBytes)(16).toString("hex");
var newSpanId = () => (0, import_node_crypto.randomBytes)(8).toString("hex");
...
function resolveSubstrateRoot() {
  ...
  for (; ; ) {
    ...
    if (fs2.existsSync(path.join(cand, "package.json"))) {
      substrateRoot = cand;
      return cand;
    }
...
function resolveKitBinJs(bin) {
  ...
  if (subRoot) {
    if (substrateBinMap === null) {
      try {
        const pkg = JSON.parse(fs2.readFileSync(path.join(subRoot, "package.json"), "utf8"));
...
function detachedHttpSink(opts) {
  const spawnImpl = opts.spawnFn ?? import_node_child_process2.spawn;
  ...
  return {
    deliver(payload) {
      try {
        const body = payload.spans.map((s) => JSON.stringify(wireLine(s, payload))).join("\n") + "\n";
        const file = (0, import_node_path2.join)(dir, `consort-telemetry-${(0, import_node_crypto2.randomUUID)()}.ndjson`);
