...
function readSmellsLog(consortDir) {
  const file = (0, import_path2.join)(consortDir, "smells.json");
  if (!(0, import_fs2.existsSync)(file)) return { detected: [] };
...
function resolveOpenSmells(consortDir, smell, opts) {
  const file = (0, import_path2.join)(consortDir, "smells.json");
  if (!(0, import_fs2.existsSync)(file)) return 0;
  const log = JSON.parse((0, import_fs2.readFileSync)(file, "utf8"));
  let n = 0;
  ...
  if (n) (0, import_fs2.writeFileSync)(file, JSON.stringify(log, null, 2) + "\n");
  return n;
}
...
function storyCycles(consortDir, featureId, story) {
  const base = (0, import_path3.join)(cyclesRootDir(consortDir), featureId, story);
