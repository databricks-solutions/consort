...
// consort/orchestrator/diagnose/collect-bundle.ts
init_esm_shims();
...
function collectDiagnosticSources(consortDir) {
  const sources = [];
  try {
    for (const f of fs2.readdirSync(escalationsDir(consortDir)).filter((n) => n.endsWith(".json")).sort()) {
      sources.push({ kind: "escalation", path: path2.join(escalationsDir(consortDir), f), exists: true });
    }
  } catch {
  }
  ...
  const fixed = [
    ["workflow-state", path2.join(consortDir, "workflow-state.json")],
    ["agent-log", path2.join(consortDir, "agent-log.jsonl"), 300],
    ["drive-live", path2.join(consortDir, "drive-live.log"), 500]
  ];
  for (const [kind, p, tail] of fixed) {
    sources.push({ kind, path: p, exists: fs2.existsSync(p), ...tail ? { tailLines: tail } : {} });
  }
  return sources;
}

// consort/orchestrator/diagnose/analyze-failure.ts
init_esm_shims();
...
// consort/orchestrator/diagnose/analyze-failure.ts
function classify(source, hasGreenFailure) {
...
function readGreenFailures(consortDir) {
  ...
  const walk = (dir) => {
    ...
    for (const e of entries) {
      ...
      if (e.isDirectory()) walk(p);
        ...
        try {
          const gf = JSON.parse(fs7.readFileSync(p, "utf8"));
          out.push({
            location: path5.relative(root, path5.dirname(p)),
            summary: gf.summary ?? "",
...
function analyzeFailure(consortDir) {
  const escalations = readEscalations(consortDir).filter((e) => !e.resolved_at).map((e) => ({
    id: e.id,
    source: e.source,
    reason: e.reason,
  ...
  const location = escalations[0]?.feature_id || greenFailures[0]?.location ? [escalations[0]?.feature_id, escalations[0]?.story_id].filter(Boolean).join("/") || greenFailures[0]?.location : void 0;
  return {
    hasFailure,
    escalations,
    greenFailures,
...
// consort/orchestrator/diagnose/redact.ts
init_esm_shims();
var RULES = [
...
// bin/consort/diagnose.cli.ts
var CONSORT_ISSUES_URL = "https://github.com/databricks-solutions/consort/issues";
...
function redactedCopy(src, dst, tailLines) {
  let content = fs8.readFileSync(src, "utf8");
  if (tailLines) {
    const lines = content.split("\n");
    content = (lines.length > tailLines ? lines.slice(-tailLines) : lines).join("\n");
  }
  fs8.writeFileSync(dst, redactSecrets(content));
}
function bundleName(consortDir, s) {
  const rel = path6.relative(consortDir, s.path).replace(/[\\/]/g, "__");
  return rel || path6.basename(s.path);
}
function main() {
...
`);
  ...
  const outDir = args.out ?? path6.join(consortDir, "diagnostics", ts);
  fs8.mkdirSync(outDir, { recursive: true });
  const collected = [];
  for (const s of present) {
    const name = bundleName(consortDir, s);
    const dst = path6.join(outDir, name);
    redactedCopy(s.path, dst, s.tailLines);
    collected.push({ kind: s.kind, from: path6.relative(consortDir, s.path), file: name, ...s.tailLines ? { tailed: s.tailLines } : {} });
  }
  const manifest = {
  ...
  fs8.writeFileSync(path6.join(outDir, "manifest.json"), JSON.stringify(manifest, null, 2) + "\n");
