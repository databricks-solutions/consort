...
- **Repoints scm-utils `v0.2.36` -> `v0.2.39`.** Carries: the provisioning guard for `createLakebaseProject` (v0.2.37 wrongly required a project-level `current_state` the API never returns and broke all creation + the extension adopt flow; v0.2.38 fixed it to key on project PRESENCE, still fail-closed on a genuinely absent project); the scaffolded `pyproject.toml` Starlette `<1.0` cap (unbreaks FastAPI `include_router` on a fresh clone); local client + root E2E free-port allocation in `run-tests.sh`; the five substrate scripts resolving via `./scripts/lk` instead of a root `node_modules/.bin` a scaffolded project never has (v0.2.39); and the original `mergePaired` force-checkout past disposable `.consort`/`.lakebase` churn that unblocks per-story accept/merge (v0.2.36).
- **`consort-drive` refuses a feature drive when NO feature branch is claimed.** Branch claiming is owned by the command (`/sprint`, `/design`, `/build`, `/deploy` all claim before driving); invoking `consort-drive --feature X` directly (e.g. to bound `--only design`) with no claim left the working tree on the tier parent, so the design/build lane wrote its artifacts onto staging with no paired branch. The driver now fails closed and points at `lakebase-scm-claim-feature-branch` (the foreign-claim guard already covered a *different* claim; this covers *no* claim). Guarded (`drive-foreign-claim-guard.test.ts`).
- **Scaffolded substrate scripts converged.** The five substrate scripts (`refresh-token.sh`, `connect-main-branch.sh`, `delete-lakebase-branches.sh`, `set-production-db-secrets.sh`, `sanitize-branch-name.sh`) route bin resolution through `./scripts/lk` via a shared `resolve-scm-bin.sh` (no root `node_modules/.bin` probe). (`run-tests.sh` is intentionally NOT converged here: consort uses a block-less base + E2E-append-on-upgrade model, distinct from the substrate's inline runtime-gated blocks; unifying the two is deferred.)

- **Spec Author no longer re-proposes shipped features at `/plan`.** The `propose` step was seeded only `product-overview.md` + `nfrs.md` (the PO's forward-worded standing intent, no record of what shipped), so a re-plan read it greenfield and re-proposed delivered features (observed: re-proposing F1, deferring the split it should have picked up). `deliveredFeatures()` (features whose every story is done + accepted, via `feature-status`) now seeds an `ALREADY DELIVERED` directive into the propose prompt; empty on a first sprint. `spec-author.md` gains the canon. Guarded (`consort-feature-status.test.ts`, `orchestrator-effects.test.ts`).
- **E2E seed-backend guard: a port-swapped seed URL is flagged before it hangs CI.** An authored E2E that derives the backend by string-replacing the client port (`baseURL.replace("5173","8000")`) hardcodes the backend port; a shared CI runner bumps the backend off a busy 8000 and the seed then POSTs to a stale server (2xx, so it doesn't throw) while the app reads the resolved port, so rows never render — GREEN locally, RED only in CI. `checkE2eSeedBackendDerivation` flags it via the navigator self-check; canon added (`navigator.md`, `test-strategy.md`) to seed through `${baseURL}/api/...`.
- **Dashboard: `Shipped` no longer re-lights after the `shipped → plan` loop.** `passed.has("promote")` is sprint-agnostic, so once sprint 1 promoted, an idle moment in a later sprint's Plan relit Shipped while Plan was active. Gated on the playhead not having looped back (`WorkflowGraph`), with a regression test.
- **Dashboard: turn correlation is story-scoped, so a phantom `phase.start` no longer cascades an off-by-one.** A died/retried turn that recorded no `invoke-role` turn slid every later pairing for that role across story boundaries (wrong-transcript drill-downs + a dropped `#ord`). `correlate.ts` now pairs per `(role, story)`; byte-identical on a healthy corpus (differential-vs-original test holds).
- **Dashboard: a `green-superseded` (supersession) turn lights `b-perm`, not `b-green`.** `b-green`'s `buildModeNot` omitted `green-superseded`, so a supersession turn matched b-green first (declared earlier) and its own step never lit. Added to the exclusion list; regression test over every driver build-outcome.
- **Scaffold drift: consort's upgrade/drift script copies converged to the scm-utils scaffold source.** They had drifted both ways — consort's copies lacked the seed-step `PYTHONPATH` fix (a bare `python scripts/seed_dev.py` fails `import app` with `ModuleNotFoundError`), the `post-checkout` root-dep self-heal, the run-tests orphan-client-tests false-green guard, and pointed the last-resort dist path at `@databricks-solutions/consort` instead of the owning `lakebase-scm-utils`. All reconciled so `consort-upgrade` delivers them.
- **`lib/planning.test.ts` no longer crashes the dashboard suite to collect on a corpus-absent checkout.** `describe.skipIf` still runs its callback body, so `join(CORPUS!, …)` threw when the marketplace corpus was absent (CI / fresh clone / the pre-push gate). Guarded so collection is safe; the tests stay skipped.

### Changed

- **The project overview seed is narrative-only.** `stockflow-seed/intake/product-overview.md` carried a large `## Architectural requirements` block (stack / layering / migrations / testing) — implementation detail that per PO canon lives in `nfrs.md` (gate-enforced), `design-brief.md`, and the substrate gates, not the PO's open-ended overview. Removed it; nothing load-bearing lost (zero replay cascade — `nfrs.md`/`design-brief.md` untouched). A sprint-N redraft that drops it is now canon-correct, not a regression.
- **CI: `apps/dashboard` is gated on its own suite.** The vendored dashboard is a separate package (own `vitest`/`tsconfig`, ~408 tests) that root `npm test` never ran. `.husky/pre-push` now runs the dashboard `tsc --noEmit` + `vitest run` (the only automated gate while org Actions are disabled), and `ci.yml` gained a matching `dashboard` job for when Actions are enabled. The spec-author's own agent-side self-check (`consort-response-formatter`) now runs the SAME registered-breakdown check the spec gate does — at the breakdown turn (story-slug drift) and each per-story turn (that story's AC-slug drift) — so a renamed/invented/dropped story or AC fails the author's precheck IN-TURN and it re-authors the registered ids, instead of the design lane drifting and walling the gate turns later (observed: stockflow deriving `S2-stock-home-screen` / renamed S1 ACs against its registration). DRY: reuses the gate's `readRegistration`/`checkRegisteredBreakdown`/`readDerivedBreakdown`; no-op for an unregistered feature. `spec-author.md` gains the rule (read `registration.json`, mirror its exact ids).
- **Dashboard: the active-step highlight no longer jumps after a `reasoning` event.** A driver `reasoning` line (which carries no phase/buildMode) fell into the base GREEN step while a REFACTOR turn was in force, so the highlight flipped refactor→green. Fixed centrally in the ONE place the active step's turn-identity is carried: `eventsWithFeature`/`withCarried` now carry `buildMode` forward (reset at each `phase.start`), so a narration event holds its turn's step (refactor stays refactor; review stays review) and a base-green turn never inherits a prior refactor's mode. Covers every "jump after reasoning" case, not just refactor→green.
- **Dashboard: the step-card metric row (`model·effort·turns`) fits the card and reads tight.** Abbreviated the wide effort labels (`medium→med`, `minimal→min`, `xhigh→xhi`), joined the parts with a bare middot (the mono `" · "` read as big word gaps), and added fit-to-width so a long metric (e.g. `sonnet·med·10 turns`) condenses instead of spilling past the card edge.

### Changed

- **Dashboard: removed the redundant bottom "Open issues" section; its detail folds into the Orchestrator status card.** The card already turns red on an escalation; it now also shows the top blocker's source, reason, and resolver role (`→ fix by NAVIGATOR`) there, so the issue's "what + who" lives in the ONE issue surface instead of a duplicate section.

- **The one-time install beacon now retries reliably (it was trapped in the first-run block).** The beacon (a disclosed, opt-out-independent "an install exists" marker: random id + version + date) was sent only by the `/consort:start` first-run disclosure step, which runs only while `acknowledged` is false and never re-runs after acknowledgment. So a first beacon POST that missed (e.g. an Azure cold start over the client's 5s timeout) never retried, `beacon_sent` stayed false, and the install — even as it ran many drives — was never counted (`telemetry.installs` read 6 vs 10 real installs). `beginTelemetryRun` now fires the beacon best-effort on EVERY run (before the consent gate, so it still fires when a user opted out of run telemetry — the beacon is disclosed and opt-out-independent), gated on `acknowledged` (never before disclosure) and idempotent via `beacon_sent`, so a missed first POST retries until it lands. Complements the ingest-side fix that derives `installs` from run traces. Regression-guarded (`install-beacon.test.ts`).
- **The design-fingerprint stale-experiment guard no longer false-positives when the build greens a test.** `storyDesignFingerprint` hashed the ENTIRE per-story test-list, including each item's mutable `status`. The build rewrites the test-list in place (each test's `status` moves `pending -> green`), so the fingerprint shifted mid-build and the drive's `experimentStale` check flagged the STILL-VALID experiment as stale — deriving a `cut-experiment --reset-stale-branch` that DISCARDS the just-built GREEN work, on every experiment the moment its first test greened (observed live: stockflow-3-88 S2 with T17–T20 all green, design byte-identical). The fingerprint now projects the test-list to its DESIGN identity (id / ac_id / kind / layer / nfr_id / description / scenario_file) and excludes build-mutable fields (`status`, and defensively `green_at`/`cycle_ids`/`cycles`/`last_run`/`updated_at`), so only a genuine redesign moves it. Regression-guarded (`consort-design-fingerprint.test.ts`: stable across `pending -> green`, still changes on an `ac_id` redesign). NOTE: the fingerprint value changes with this fix, so an experiment stamped by an older kit is flagged stale ONCE on upgrade — reconcile its recorded `design_fingerprint` if it carries unmerged work.
- **E2E route mocks no longer collide with the app's own ES modules under Vite (the T20 SPA-boot dead-lock).** A Playwright spec that mocked an API with a broad glob — `page.route("**/api/stock**", ...)` — also matched the app's own module served by Vite at `/src/api/stock.ts`: the mock fulfilled that ES-module request with JSON, the browser rejected it (`MIME type application/json`), the SPA never booted, and the behavior under test (an empty-state) could never render. The app was correct — the TEST was the defect — so no product change made it GREEN and the cycle dead-locked (two reopen cycles could not converge). Fixed on the real path: (1) the mock-policy guidance (`test-strategy.md`) + the navigator's Playwright rules now require scoping a `page.route` mock to the API PATHNAME (`(url) => new URL(url).pathname === "/api/stock"`), never a `**`-glob; (2) a deterministic guard — `checkE2eRouteCollision` (`architecture/e2e-route-adherence.ts`), surfaced as the `navigator` scope of the agent-side `consort-response-formatter` — flags an authored `page.route` glob whose Playwright match also catches a `client/src/**` module's dev URL, with the exact remediation, so the navigator fixes its RED spec in-turn (a URL-matcher function and an anchored non-glob string are never flagged; a backend-only project is a clean no-op).
- **The experiment-cut fork-parent guard no longer false-positives on a runtime-artifact-only tip.** The GIT<->DB fork-parent agreement guard (`experiment.ts`) used a git-ANCESTRY proxy — HEAD must descend from the local `parentBranch` tip — to catch a schema/code split-brain (the git fork-parent missing migrations/models the Lakebase tier already has). But a legitimate reopen/re-author leaves a trivial runtime-artifact commit (a `pipeline.json` gate-status line / `.consort` corpus churn) ahead of the tier commit the fork lands on; that tripped the proxy and aborted the cut even though there is NO schema/code delta — and pushing the branch does not help, since the git side forks from the tier commit, not origin. Replaced the proxy with the true invariant: `forkParentAgreementReason` diffs HEAD against the tip and halts ONLY when the divergence touches a schema/code file, reusing the SAME `RUNTIME_ARTIFACT_PREFIXES` set the pre-fork dirty guard trusts (one owner for "what does not corrupt a fork"). Fail-closed if the diff cannot be computed; the halt message now names the diverging source files. Live-proven against the real blocked run, hermetically tested on real temp git repos.
- **The registered-breakdown guard now also runs on the per-story spec gate (the interactive path).** v0.3.88 put `registeredBreakdownReason` on `resolveArtifactInputs` (the human-proxy / feature-level path), but an interactive run approves each story's spec through `approveStoryGateFromDisk` (`story-pipeline.ts`), which the feature-level check does not cover — so a divergent per-story breakdown could still pass. Added the same fail-closed `registeredBreakdownReason` HARD-BLOCK to `approveStoryGateFromDisk`, proven on the per-story path against real divergent data.
- **The Consort MCP server launches from any cwd (fixes "Connection closed").** `.mcp.json` launched the server with a bare relative path (`./dist/apps/mcp-server/index.js`); Claude Code launches a plugin's MCP server from a cwd that is NOT the plugin dir, so `node ./dist/...` threw module-not-found and the stdio connection closed at once. Now resolved via `${CLAUDE_PLUGIN_ROOT}` (Claude Code substitutes the installed plugin dir; an external client replaces it with the absolute install path). Regression-guarded (`mcp-launch-config.test.ts`).
