...
- `.consort/features/<F>/test-list.json` – Beck's master ordered list at the **feature** level, the source of truth. **When invoked for a single story, APPEND that story's tests to the master, preserve every other story's items, and never author a `test-list-per-story.json` (the per-story + per-AC views are generated from the master; a file you write is regenerated and lost).** Write EXACTLY this shape (ordered tests in a top-level `items` array, NOT `tests`; no other top-level keys, a renamed/extra key fails the gate):
  ...
  **Premise persistence items on the CURRENT shipped schema.** Before authoring an item about a constraint/index/column, check whether the base schema or a MERGED feature's migration already delivers it (read the existing migrations first). If it does, the item asserts the BEHAVIOR against the real branch DB (a violating write is rejected) – NEVER that this story adds a NEW migration for the already-shipped invariant. A "story adds a migration for X" item where X is already shipped is mis-premised: it can only be greened by rewriting schema history (issue #196). Migration-introduction items belong only to schema this feature itself adds.
  **Author items the Navigator can write a SOUND test from (the issue #199 catalog).** Each item's description should demand: assertions scoped to a UNIQUE KEY (a row by its sku/id) with EXACT matches – never bare short literals or broad alternations near uuid-seeded data (they substring-match sibling cells and UUID digits: green per-AC, RED at full-suite verify); no DELETE-based teardowns (append-only / ON DELETE RESTRICT tables block them – isolation comes from per-run uuid keys, not cleanup); no umbrella `except IntegrityError` (it masks the real failure); and raw-SQL references only to tables a migration actually creates (verify the name in the migrations – plurals matter).
